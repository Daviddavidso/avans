<?php
/* ==========================================================================
   БЭКЕНД ПАНЕЛИ УПРАВЛЕНИЯ «АВАНС». Один файл, нужен только PHP.
   ...
   Служебные файлы (заявки, файлы бота) лежат в папке .avans-private НАД
...
/* На витрине нет формы заявки, поэтому открытый приём заявок выключен:
   иначе api.php оставался бы адресом, куда кто угодно пишет в leads.csv.
   Появится форма — поставьте true, остальной код уже готов. */
...
session_name('avans_admin');
session_start();

/* ---------------------------------------------------------- закрытая папка */

$PRIV = dirname($dir) . '/.avans-private';
...
if ($action === 'lead') {
    if (!LEADS_ENABLED) out(['ok' => false, 'error' => 'Приём заявок выключен.'], 404);
